...
const validateInstructor = ({ avatar, name, about, gender }) => {
  if (!name.trim()) return { error: "Instructor name is missing!" };
  if (!about.trim()) return { error: "About section is empty!" };
  if (!gender.trim()) return { error: "Instructor gender is missing!" };
...
export default function InstructorForm({
  ...
  const handleChange = ({ target }) => {
    ...
    if (name === "avatar") {
      ...
      return setInstructorInfo({ ...instructorInfo, avatar: file });
    }
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const { error } = validateInstructor(instructorInfo);
    if (error) return updateNotification("error", error);

    // submit form
    const formData = new FormData();
    for (let key in instructorInfo) {
      if (key) formData.append(key, instructorInfo[key]);
    }
    onSubmit(formData);
  };
  ...
  useEffect(() => {
    if (initialState) {
      setInstructorInfo({ ...initialState, avatar: null });
      setSelectedAvatarForUI(initialState.avatar);
    }
  }, [initialState]);
  const { name, about, gender } = instructorInfo;
